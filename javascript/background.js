...
const B_SCRIPT_TAG      =  B_SCRIPT_ID +" manifest "+MANIFEST_VERSION+" (230911:15h:39)"; /* eslint-disable-line no-unused-vars */
...
"use strict";
    ...
    // └───────────────────────────────────────────────────────────────────────┘
    try {
        console.log("Trying importScripts (works in Chrome with manifest v3, !(type module)...");
        importScripts("log.js");
        log_js.log("● importScripts('log.js') worked ●");
    }
    // ┌───────────────────────────────────────────────────────────────────────┐
    // │ ● importScripts ● fail                                                │
    // └───────────────────────────────────────────────────────────────────────┘
    catch(ex) {
        log_js.log("● importScripts('log.js') did not work ●");
        console.log(ex.message);
    }
    // ┌───────────────────────────────────────────────────────────────────────┐
    // │ ● log.js ● from importScripts or manifest background.script[]         │
    // └───────────────────────────────────────────────────────────────────────┘
    console.log("● log_js:", log_js);
    if(log_js) log = log_js.log;
...
/*_ ohr_listener_get_details_responseHeaders_CSP {{{*/
    ...
    let l = function()
    {
        let s = ""; Object.keys(b).forEach((f) => s += "\n● b."+f+((typeof b[f] == "function") ? "()":""));

        console.log(B_SCRIPT_ID+" Devtools console commands:"+ s);
    };
    /*}}}*/
